...
class OBSWoocommerceCustomSoft1
{
    ...
    public function obs_woocommerce_custom_soft1_get_products_stock()
    {
        ...
        if (count($stocks)) {
            ...
            $skus = implode(',', array_keys($stocks));

            $data = [];

            $results = $wpdb->get_results($wpdb->prepare("SELECT post_id, meta_value as mtrl FROM $wpdb->postmeta WHERE meta_key='MTRL_MTRSUBSTITUTE_CODE' order by post_id DESC"));
            foreach ($results as $result) {
                if (isset($stocks[$result->mtrl])) {
                    if (isset($_GET['print_stock'])) {
                        echo $result->post_id . ' - ' . $stocks[$result->mtrl] . '<br>';
                    } elseif (isset($_REQUEST['export_stock_csv'])) {
                        $data[] = ['MTRL_MTRSUBSTITUTE_CODE' => $result->mtrl, 'product_id' => $result->post_id, 'stock' => $stocks[$result->mtrl]];
                    } elseif (isset($_REQUEST['export_stock_raw_csv'])) {
                        $data[] = ['MTRL_MTRSUBSTITUTE_CODE' => $result->mtrl, 'product_id' => $result->post_id, 'stock' => $stocks[$result->mtrl]];
                    } else {
                        $product = wc_get_product($result->post_id);
                        $product->set_stock_quantity(wc_stock_amount($stocks[$result->mtrl]));
                        $product->set_stock_status(intval($stocks[$result->mtrl]) > 0 ? 'instock' : 'outofstock');
                        $product->save();
                    }
                } else {
                    $product = wc_get_product($result->post_id);
    ...
    public function insert_product($product_data)
    {
        ...
        $sizes = array_unique(array_filter(array_map(function ($product) {
            if (isset($product['Sz'])) return $product['Sz'];
        }, $product_data)));
        $colors = array_unique(array_filter(array_map(function ($product) {
            if (isset($product['COL'])) return $product['COL'];
        }, $product_data)));
        ...
        if (isset($product_data[0]['CtgA']) && $product_data[0]['CtgA']) {
            ...
            if ($term = get_term_by('slug', $this->slugify($value), 'product_cat', 'ARRAY_A')) {
                $parent_id = $term['term_id'];
                $categories[] = $parent_id;
                if (isset($product_data[0]['CtgB']) && $product_data[0]['CtgB']) {
                    $value = $product_data[0]['CtgB'];
                    $sub_categories = get_categories(
                        array('parent' => $parent_id, 'taxonomy' => 'product_cat')
                    );
                    foreach ($sub_categories as $sub_category) {
                        if ($sub_category->name == $value) {
                            $sub_parent_id = $sub_category->term_id;
                            $categories[] = $sub_parent_id;
                            if (isset($product_data[0]['CtgC']) && $product_data[0]['CtgC']) {
                                $sub_value = $product_data[0]['CtgC'];
                                $sub_sub_categories = get_categories(
                                    array('parent' => $sub_parent_id, 'taxonomy' => 'product_cat')
                                );
                                foreach ($sub_sub_categories as $sub_sub_category) {
                                    if ($sub_sub_category->name == $sub_value) {
                                        $sub_sub_parent_id = $sub_sub_category->term_id;
                                        $categories[] = $sub_sub_parent_id;
                                        if (isset($product_data[0]['CtgD']) && $product_data[0]['CtgD']) {
                                            echo $sub_sub_value = $product_data[0]['CtgD'];
                                            $sub_sub_sub_categories = get_categories(
                                                array('parent' => $sub_sub_parent_id, 'taxonomy' => 'product_cat')
                                            );
                                            foreach ($sub_sub_sub_categories as $sub_sub_sub_category) {
                                                if (htmlspecialchars_decode($sub_sub_sub_category->name) == $sub_sub_value) {
                                                    $categories[] = $sub_sub_sub_category->term_id;
                                                }
                                            }
                                        }
                                    }
                                ...
                                foreach ($sub_sub_categories as $sub_sub_category) {
                                    if ($sub_sub_category->name == $sub_value) {
                                        ...
                                        if (isset($product_data[0]['ALCD']) && $product_data[0]['ALCD']) {
                                            echo $sub_sub_value = $product_data[0]['ALCD'];
                                            $sub_sub_sub_categories = get_categories(
                                                array('parent' => $sub_sub_parent_id, 'taxonomy' => 'product_cat')
                                            );
                                            foreach ($sub_sub_sub_categories as $sub_sub_sub_category) {
        ...
        foreach ($product_data as $product) {
            $variation_data = [];
            if (isset($product['Sz'])) {
                $variation_data['sizes'] = (string) urldecode(sanitize_title($product['Sz']));
            }
            if (isset($product['COL'])) {
                $variation_data['colors'] = (string) $product['COL'];
            }
            if (isset($product['MTRSUP_SUPPLIER_NAME'])) {
                $variation_data['brands'] = (string) $product['MTRSUP_SUPPLIER_NAME'];
            }
        ...
        if (count($sizes)) {
            $available_attributes[] = ['variation' => true, 'value' => 'sizes'];
        }

        if (count($colors)) {
            $available_attributes[] = ['variation' => true, 'value' => 'colors'];
        }
        $available_attributes[] = ['variation' => false, 'value' => 'brands'];


        $post = array( // Set up the basic post data to insert for our product

            'post_author'  => 1,
            'post_content' => $product_data[0]['WEBNAME'],
            'post_status'  => 'publish',
            'post_title'   => $product_data[0]['WEBNAME'],
            'post_parent'  => '',
            'post_type'    => 'product'
        );
        ...
        $posts = get_posts([
            'post_type' => 'product',
            'meta_key' => 'MTRL',
            'meta_value' => $product_data[0]['MTRL'],
        ]);
        if (count($posts)) {
            $post_id = $posts[0]->ID;
            echo '$post_id=' . $post_id . '<br>';
            wp_update_post(array_merge($post, ['ID' => $post_id]));
        } else {
        ...
        {
            //die("no post id something has gone wrong so don't proceed");
            return false;
        }
        ...
        update_post_meta($post_id, '_sku', $product_data[0]['CODE']); // Set its SKU
        update_post_meta($post_id, 'mtrl', $product_data[0]['MTRL']); // Set its SKU
        update_post_meta($post_id, '_visibility', 'visible'); // Set the product to visible, if not it won't show on the front end

        wp_set_object_terms($post_id, $categories, 'product_cat'); // Set up its categories
        wp_set_object_terms($post_id, 'variable', 'product_type'); // Set it to a variable product type

        $this->insert_product_attributes($post_id, $available_attributes, $variations); // Add attributes passing the new post id, attributes & variations
        $this->insert_product_variations($post_id, $variations); // Insert variations passing the new post id & variations
    }
    ...
    public function insert_product_attributes($post_id, $available_attributes, $variations)
    {
        ...
        {
            if (!isset($this->terms['pa_' . $attribute['value']])) {
                $this->terms['pa_' . $attribute['value']] = get_terms(array(
                    'taxonomy' => 'pa_' . $attribute['value'],
                    'hide_empty' => false,
                ));
            }
            ...
            foreach ($variations as $variation) // Loop each variation in the file
            {
                ...
                {
                    if ($key === $attribute['value']) // If this attributes key is the top level attribute add the value to the $values array
                    {
                        $found = false;
                        foreach ($this->terms['pa_' . $attribute['value']] as $term) {
                            if ($term->name == $variation['attributes'][$key]) {
                                $values[] = $term->slug;
                                $found = true;
                                break;
                            }
            ...
            wp_set_object_terms($post_id, $values, 'pa_' . $attribute['value']);
        }
        ...
        {
            $product_attributes_data['pa_' . $attribute['value']] = array( // Set this attributes array to a key to using the prefix 'pa'

                'name'         => 'pa_' . $attribute['value'],
                'value'        => '',
                'is_visible'   => '1',
                'is_variation' => (string) boolval($attribute['variation']),
                'is_taxonomy'  => '1'
    ...
    public function insert_product_variations($post_id, $variations)
    {
        foreach ($variations as $index => $variation) {
            ...
            $posts = get_posts([
                'post_type' => 'product_variation',
                'meta_key' => 'MTRL_MTRSUBSTITUTE_CODE',
                'meta_value' => $variation['MTRL_MTRSUBSTITUTE_CODE'],
            ]);
            ...
            if (count($posts)) {
                $variation_post_id = $posts[0]->ID;
                // echo '$variation_post_id='.$variation_post_id.'<br>';
                wp_update_post(array_merge($variation_post, ['ID' => $variation_post_id]));
            } else {

                $variation_post_id = wp_insert_post($variation_post); // Insert the variation

                update_post_meta($variation_post_id, '_stock', 1);
                update_post_meta($variation_post_id, '_stock_status', wc_clean('instock'));
                wp_set_post_terms($variation_post_id, 'instock', 'product_visibility', true);
                // And finally (optionally if needed)
                wc_delete_product_transients($variation_post_id); // Clear/refresh the variation cache
            }
            ...
            foreach ($variation['attributes'] as $attribute => $value) // Loop through the variations attributes
            {
                ...
                if ($attribute_term->slug) {
                    ...
                    $attribute_term = get_term_by('name', strtolower($value), 'pa_' . $attribute); // We need to insert the slug not the name into the variation post meta
                    if ($attribute_term->slug) {
                        update_post_meta($variation_post_id, 'attribute_pa_' . $attribute, $attribute_term->slug);
                    }
                    update_post_meta($variation_post_id, 'attribute_pa_' . $attribute, strtolower($value));
                }
            }

            update_post_meta($variation_post_id, 'MTRL_MTRSUBSTITUTE_CODE', $variation['MTRL_MTRSUBSTITUTE_CODE']);
            ...
            update_post_meta($variation_post_id, '_price', $variation['sale_price'] ?? $variation['regular_price']);
            // And finally (optionally if needed)
            wc_delete_product_transients($variation_post_id); // Clear/refresh the variation cache
        }
    }
...
<?php }
    ...
    {
        /*
	    foreach([63209, 63208, 63207, 63206, 63205, 63204, 63203, 63202, 63201, 63200, 63199, 63197, 63196, 63195, 63194, 63193, 63192, 63875, 63874, 63873, 63872, 63870, 63869, 63868, 63867, 63866, 63865, 63864] as $id){
